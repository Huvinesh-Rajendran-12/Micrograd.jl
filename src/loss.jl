function mean_squared_error(predicted::Vector{Value}, actual::Vector{Float64})
...
function mean_absolute_error(predicted::Vector{Value}, actual::Vector{Float64})
    @assert length(predicted) == length(actual) "Predicted and actual must have the same length"
    return sum(abs.(predicted.data .- actual)) / length(predicted)
end

function binary_cross_entropy(predicted::Vector{Value}, actual::Vector{Float64})
    @assert length(predicted) == length(actual) "Predicted and actual must have the same length"
    epsilon = 1e-10  # Small value to avoid log(0)
    return sum(-(actual .* log.(predicted.data .+ epsilon) + (1 .- actual) .* log.(1 .- predicted.data .+ epsilon))) / length(predicted)
end

function categorical_cross_entropy(predicted::Vector{Value}, actual::Vector{Int})
    @assert length(predicted) == length(actual) "Predicted and actual must have the same length"
    epsilon = 1e-10  # Small value to avoid log(0)
